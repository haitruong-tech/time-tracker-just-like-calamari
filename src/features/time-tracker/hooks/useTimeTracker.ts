...
function useTimeTracker (): IUseTimeTrackerReturnValue {
  ...
  const work = (): void => {
    if (timers.at(-1)?.purpose === PURPOSES.WORK || lastUpdate.current === undefined || lastUpdate.current === null) return
    dispatch({
      type: RECORD,
      payload: {
  ...
  const takeBreak = (): void => {
    if (timers.at(-1)?.purpose === PURPOSES.BREAK || lastUpdate.current === undefined || lastUpdate.current === null) return
    dispatch({
      type: RECORD,
      payload: {
  ...
  useEffect(() => {
    lastUpdate.current =
      new Date(JSON.parse(localStorage.getItem(LOCAL_STORAGE.LAST_UPDATE) as string)) ??
      new Date()

    function updateProgress (): void {
      if (lastUpdate.current === undefined || lastUpdate.current === null) return
      dispatch({
